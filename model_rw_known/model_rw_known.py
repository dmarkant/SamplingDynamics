"""
Version of random walk model with known sampling sequence.

Basic point of difference is that unlike in original form,
where the sampling sequence is a latent process, here the
set of samples are assumed to be known. However, assume the
same architecture otherwise.

On each trial, transition probabilities are based on the
observed value of the sample (relative to the pooled variance
of the problem).
...
def drift(obs, options, v, delta, gamma, pow_gain, pow_loss, w_loss):
    """
    v -- current state
    """
    ...
    u = util(outcome, pow_gain, pow_loss, w_loss)
    dr = delta * u / 1.
...
def transition_probs(v, obs, delta, tau, gamma, alpha, options, pow_gain, pow_loss, w_loss):
    ...
    """
    dr = drift(obs, options, v, delta, gamma, pow_gain, pow_loss, w_loss)
...
def transition_matrix_PQR(V, dv, obs, delta, tau, gamma, alpha, options, pow_gain, pow_loss, w_loss):
    ...
    for i in range(1, m - 1):
        ...
        tm_pqr[row, ind_pqr] = transition_probs(i*dv, obs, delta, tau, gamma, alpha, options, pow_gain, pow_loss, w_loss)
...
def run(pars):
    ...
    # starting position
    if 'Z' in pars:
        Z = np.matrix(pars.get('Z')) # provided directly
    ...
    for trial in range(len(data['outcomes'])):

        sampled = data['samples'][trial]
        outcome = data['outcomes'][trial]


        transition_matrix_PQR(V, dv, [sampled, outcome], delta, tau, gamma, alpha, options, pow_gain, pow_loss, w_loss)
